...
import (
	...
	"github.com/akhamatvarokah/miniWallet/backend/security"
...
func (u *User) FindOtherUsers(db *gorm.DB, id uint64) (*[]User, error) {
	var err error
	users := []User{}
	err = db.Debug().Model(&User{}).Where("id != ?", id).Limit(100).Find(&users).Error
	if err != nil {
		return &[]User{}, err
	}
	return &users, err
